...
module Maze
  class Builder
    ...
    attr_accessor  :maze, :path, :visited_rooms
    def initialize(options)

      @maze = options[:maze]
      @path = [Position.new( rand(1..maze.columns ), rand(1..maze.rows))]
      @visited_rooms = @path.dup
    end
    ...
    def previous_position
      visited_rooms[visited_rooms.length - 2]
    end

    def go_back_to_previous_visited_room
      visited_rooms.pop if visited_rooms.length >= 1
    end
    ...
    def build_maze_like_a_robot
      while !maze.all_rooms_visited? do
        ...
        if !valid_rooms_to_build.empty?
          ...
          visited_rooms << next_room.position
    ...
    def build_maze
      while !maze.all_rooms_visited? do
        ...
        if !valid_rooms_to_build.empty?
          ...
          random_unvisited_room = maze.rooms.select {|room| !room.visited?}.sample
           if random_unvisited_room
             visited_rooms.pop
             visited_rooms << random_unvisited_room.position
              path << current_room.position
              binding.pry
           else
             binding.pry
